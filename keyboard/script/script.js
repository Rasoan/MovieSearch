...
reference_text.innerHTML = "Привет\n" + 
                            "1) На разрешении 780px+ есть виртуальная клавиатура, что бы она подсвечивалась,\n раскладка её и раскладка твоей физической должны совпадать;\n" +
                            "_______________________________\n" +
                            "2) По основным вопросам есть тз, от себя добавлю, что дополнительным функционалом считаю\n - переключатель темы, который находится справа вверху\n - стрелки перелистывания страницы слайдера сразу, а не по 1\n - цифры показывающие на каком слайде мы находимся в данный момент и сколько всего в слайдере + отображается общее количество фильмов найденных по запросу в строке-сообщение\n - hover-эффект при котором отображаются на карточках жанр фильма и его описание;\n" +
                            "_______________________________\n" +
                            "3) После проверки моей работы оставь пожалуйста свои контакты, telegram или вк или дискорд," +
                                "что бы можно было уточнить возникшие вопросы, в этой работе много пота, позволь уточнить у тебя, подробности найденного косяка;\n" +
                                "_______________________________\n" +
                            "4) По вопросам можно и лучше нужно писать мне в телеграмм,\nAraik Rasoyan\n(+375 25 664 30 70);\n" +
                            "_______________________________\n" +
                            "Спасибо за внимание, удачи в выполнении следующих тасков.";
...
keyboardСontainer.classList.add("keyboard-container", "display-none");
...
// работа со стрелочками
document.addEventListener("click", element => {

  if ( element.target.classList.contains("key-left") && my_input_search.selectionEnd ) { // если клик по стрелке влево и есть символыв инпуте
    my_input_search.selectionEnd--;
    my_input_search.selectionStart = my_input_search.selectionEnd;  
  }

  if ( element.target.classList.contains("key-right") ) { // если клик по стрелке вправо
   my_input_search.selectionEnd++;
   my_input_search.selectionStart = my_input_search.selectionEnd;   
  }

  if ( element.target.classList.contains("key-top") ) { // если клик по стрелке вверх
    my_input_search.selectionStart = my_input_search.value.length; 
  }
...
document.addEventListener("keydown", element => {
  ...
  if ( !(element.code == "Escape") ) return; // если не нажали на esc то на этом всё
